...
class OptimizerLog:
    ...
    def update(self, step, cost, val_cost):
        ...
        print(f"train loss:{cost}", end=' ')
        train_log_txt_formatter = f"[Epoch] {step:03d} [Loss] {val_cost}\n"
        with open(train_log_filepath, "a") as f:
            #f.truncate(0)
            f.write(train_log_txt_formatter)
...
# training
'''清空上一次训练的loss信息'''
with open(train_log_filepath, "w") as f:
    f.truncate(0)
f.close()
...
if len(list(net.parameters())) != 0:
    ...
    for epoch in range(num_epochs):
        ...
        if len(log.val_costs) and log.val_costs[-1] < 1e-7:
            pass
            #break
